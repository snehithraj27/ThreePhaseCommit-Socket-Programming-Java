...
public class Participant {
	...
	JButton commitButton = new JButton("Prepare-Commit");
	...
	public Participant() { // Constructor to initialize the Participant
		...
		frame.getContentPane().add(ackButton, "South");
	...
	private void run() throws IOException, ParseException {
		...
		commitButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e2) {
				out.println("VotePreCommit,POST /localhost/server http/1.1,Host:" + serverAddress+ ",User-Agent:Mozilla/5.0,Content-Type:text,Content-Length:10,Date:" + tDate);
				messageArea.append("\nYou voted as : PreCommit\n");
				messageArea.append("\nState :PRECOMMIT\n");
				abortButton.setEnabled(false);
				commitButton.setEnabled(false);
		...
		ackButton.addActionListener(new ActionListener() {
			// Timer timer;
			public void actionPerformed(ActionEvent e2) {
				out.println("VoteAck,POST /localhost/server http/1.1,Host:" + serverAddress+ ",User-Agent:Mozilla/5.0,Content-Type:text,Content-Length:10,Date:" + tDate);
				messageArea.append("\nYou have sent the acknowledgement.\n");		
				abortButton.setEnabled(false);
				commitButton.setEnabled(false);
				ackButton.setEnabled(false);
				t=1;
				...
				// block of code is executed if message is received from Coordinator or the Participant
					...
					if (sName.contains(msg[1]) && (t == 0)) {
						...
						if (msg[1].contains("Coordinator")) { //block of code is executed if coordinator sends a message
							timer.cancel();
							messageArea.append("\nMessage from" + msg[1] + ": " + msg[2] + "\nVote either ABORT or PRE-COMMIT.\n");
							messageArea.append("\nState: READY");
							timer.cancel();
							timer = new Timer();
	...
	public static void writeFile(String msg, String name) {
		try {
			File f1 = new File("/Users/snehithraj/eclipse-workspace/ThreePhaseCommit/src/" + name);
			if (!f1.exists()) {
				f1.createNewFile();
			}
