...
public class Coordinator {
	...
	int m;
	int pc=0;// Variable m is declared to keep track of the message
	Timer timer = null;
		...
		try {
			while (true) {
				...
				// block of code is executed if message is received from participant
					...
					System.out.println(line);
					String[] msg = line.split(",");
					if (msg[1].contains("Coordinator") && t == 0&&m==1) {
						messageArea.append("\nMessage :" + msg[2]);
						commit=0;
						ack=0;
					}
					if (line.contains(sName) && t == 0 && m==1) { // Start the timer
						...
						messageArea.append("\nTimer has started for : " + timeInterval + " seconds.\n");
							// WAIT state when the coordinator sends the message and waits for the votes from the participants. 
					...
					// block of code is executed if participant votes either commit or abort
						...
						if (line.contains("Abort")) {
							...
							messageArea.append("\n" + msg[1] + " wants to PreCommit.\n");						
								commit++;
								messageArea.append("No of PreCommits: " + Integer.toString(commit) + "\n");
							if (commit == participant.size()||commit==3) {
								// global commit initiated if all the participants voted commit																	
								out.println("ack, POST/localhost/server http/1.1,Host:" + serverAddress+",User-Agent:Mozilla/5.0,Content-Type:text,Content-Length:10,Date:" + tDate); 						
								messageArea.append("\nState :PRECOMMIT.\n");
								messageArea.append("\nSending Acknowledgement Request\n");
								commit = 0;
								pc=1;
								m=0;
								timer.cancel();
								// timer is started once the coordinator enters into PRECOMMI state
								timer = new Timer();
								timer.schedule(new RemindTask1(), timeInterval * 1000);
								messageArea.append("\nTimer has started for : " + timeInterval + " seconds.\n");
							}
						}
						
						// block of code is executed if participant sends the acknowledge
						else if (line.contains("Ack")) {
							messageArea.append("\n" + msg[1] + " has sent the acknowledge.\n");						
								ack++;
								messageArea.append("No of Acknowledgements: " + Integer.toString(ack) + "\n");
							if (ack == participant.size()||ack==3) {
								// global commit initiated if all the participants voted acknowledgement																	
	...
	class RemindTask extends TimerTask {
		public void run() {
			...
			messageArea.append("\nState: INIT.\n");
			//sending global abort message to the participants
	...
	// block of code is executed if the coordinator is in PRECOMMIT state and the timer runs out.
	class RemindTask1 extends TimerTask {
		public void run() {
			...
			messageArea.append("\nTime Out.\n");
			messageArea.append("\nState: COMMIT.\n");
			messageArea.append("Initiating Global Commit\n");
			messageArea.append("\nState: INIT.\n");
			//sending the global commit message to the paticipants
			out.println("gCommit, POST/localhost/server http/1.1,Host:127.0.0.1,User-Agent:Mozilla/5.0,Content-Type:text,Content-Length:10,Date:"+ tDate1);
			timer.cancel(); 
			}
		}
